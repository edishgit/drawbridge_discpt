...
The capability that makes Drawbridge different from every "AI-powered" compliance tool: **when a vendor's uploaded document tries to prompt-inject the reviewing agents, Drawbridge blocks it, logs it, and automatically raises that vendor's risk score for attempted manipulation.** A vendor who tries to manipulate your reviewer has told you something about their trustworthiness that no questionnaire would have revealed.
...
**4. Defensible risk scoring**
- The Risk Scorer applies a weighted rubric across eight security domains to produce a 0–100 score and a band (approve / conditional / escalate).
- The model judges severity; deterministic code computes the score — so "why 71?" always has an arithmetic answer a human can inspect.
- A one-page risk memo, written for a CISO, accompanies every decision.

**5. Adversarial-conduct detection** *(the signature capability)*
- Every vendor-supplied document and reply is screened through Model Armor before any agent reads it.
- A detected manipulation attempt is not merely blocked — it raises the vendor's risk score by 25 points, forces escalation, and is preserved as evidence in the audit trail.
...
- After approval, the Watchdog agent runs scheduled sweeps over breach feeds and certificate-expiry dates for the approved-vendor portfolio, reopening a review when a vendor's posture changes.
- This directly addresses the statistic that fewer than half of organizations ever re-check a vendor.

**7. One-click audit binder**
- Every action the fleet takes is an OpenTelemetry reasoning trace; one click renders those traces into a regulator-ready evidence pack — timeline, decisions, screening results, findings, score computation, and human approvals with identity and timestamp.
...
> Three layers with three lifetimes: disposable session state, a durable Firestore workflow ledger, and cross-session Memory Bank dossiers that make the second review of a vendor smarter than the first.

**▶ IMAGE:** `diagrams/04-memory-hierarchy.png`

### Security architecture — zero trust for a fleet
> Defense in depth: content screening at ingress, policy enforcement at the gateway chokepoint, one least-privilege service account per agent, and a tamper-evident audit trail. The agent that reads evidence cannot send email; the agent that emails cannot read internal data.

**▶ IMAGE:** `diagrams/05-security-architecture.png` — includes the per-agent permission matrix.

### Resumability and idempotency
> Every side effect carries an idempotency key claimed before execution, and every workflow step is checkpointed — so a review killed mid-flight resumes from its last checkpoint and never emails a vendor twice. *(This is the "idempotency trap" the hackathon's own long-running-agents session described.)*
...
> Deployed on Vertex AI Agent Engine and Cloud Run with scale-to-zero, budget alerts, and Flash-first model routing — under $0.50 in model cost per review.
...
- **The idempotency trap.** [How you made side effects exactly-once across restarts and Pub/Sub redeliveries.]
- **Incremental questionnaire parsing.** [Handling replies that arrive partially, over days, and merging them without losing provenance.]
- **Over-flagging in cross-examination.** [Teaching the Evidence agent that missing evidence is a gap, not a contradiction, so the headline finding stays credible.]

---

## __Accomplishments that I'm proud of

- Turned a prompt-injection defense into a *product feature* — attempted manipulation becomes a risk signal, something no vendor-review tool does.
- Built a fleet that survives being killed mid-review and resumes without duplicate side effects.
- Made the audit trail the deliverable: reasoning traces export as a regulator-ready binder in one click.
- Kept model cost under $0.50 per review through Flash-first routing.
...
`gemini` · `gemini-3.5-flash` · `google-adk` · `vertex-ai` · `agent-engine` · `memory-bank` · `model-armor` · `agent-registry` · `cloud-run` · `pub-sub` · `firestore` · `cloud-storage` · `cloud-trace` · `opentelemetry` · `iam` · `secret-manager` · [`gemma` — if built] · `next-js` · `tailwindcss` · `python`
...
The repository includes the full synthetic vendor pack — including the labeled injection payload — so you can reproduce every moment in the demo, including the adversarial-conduct block.
