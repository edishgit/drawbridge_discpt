...
for i,ln in enumerate(wrap("30% of breaches now involve a third party — double last year. The only gate protecting an enterprise from a risky vendor is a 60-question spreadsheet and a three-week email thread. Drawbridge replaces it, and raises a vendor's risk score when their documents try to prompt-inject the reviewer.",108)):
...
for i,ln in enumerate(wrap("A vendor's uploaded PDF contained hidden white-on-white text instructing the reviewing agent to mark them low-risk and skip evidence checks. Drawbridge blocked it, kept it as inert evidence, and raised the vendor's risk score for attempted manipulation — because that attempt is itself a finding.",150)):
...
chips=[("All",8,False),("Needs you",2,True),("In flight",4,False),("Decided",2,False)]
cx=28
