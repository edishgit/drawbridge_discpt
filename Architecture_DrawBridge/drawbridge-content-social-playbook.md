...
**Subtitle:** *Drawbridge runs enterprise vendor-security reviews autonomously on Google's Gemini Enterprise Agent Platform — and treats manipulation attempts as risk signals. Built solo in 20 days.*
...
**§1 · Cold open — the attack (≈150 words).** Start inside the moment: a vendor's "Security_Overview.pdf" arrives; hidden in white-on-white text, instructions telling any automated reviewer to rate the vendor low-risk and skip evidence checks; the fleet blocks it and *raises their risk score.* Then the turn: "The vendor is fictional — it's from my test pack. The attack pattern is not." → **Image: real product screenshot of the adversarial banner** (mockup 02's red banner as built). → Disclosure sentence lands at the end of this section.
...
**§4 · Five decisions that mattered (≈450 words).** The heart of the piece — one short passage each: (1) every side effect routes through a gateway with two named policies (P1 human token for outbound email, P2 armor stamp for model input); (2) idempotency keys claimed *before* the side effect, so a resumed workflow never emails twice; (3) the model judges severity but plain Python computes the score from a rubric file — so "why 71?" has an arithmetic answer; (4) three memory layers with three lifetimes; (5) one service account per agent — the reviewer that reads evidence cannot email, the one that emails cannot read your data. → **Images: `04-memory-hierarchy.png`** after decision 4, **`05-security-architecture.png`** after decision 5.

**§5 · The injection defense, step by step (≈300 words).** Walk the pipeline: quarantine bucket → in-VPC scrub → Model Armor → verdict → the Adversarial Conduct flag. Explain *why* the score-raise is the interesting part, not the block. → **Image: `03-injection-defense.png`.** → **Pull quote (styled as Medium blockquote):** *"A vendor who attacks your reviewer has answered your questionnaire more honestly than they intended."*
...
**Outline:** (1) the trap of conflating "the process didn't die" with "the system remembers" — the webinar's framing, credited; (2) the three-layer model with lifetimes — **image `04-memory-hierarchy.png`**; (3) what deliberately dies in a crash and why that's fine; (4) checkpoints + claim-before-effect idempotency, with the state-machine table as the only code block; (5) the distillation rule for Memory Bank — store what's worth knowing next time, not the transcript; (6) the payoff scene: the *second* review of the same vendor, where the fleet recalls the negotiated MFA exception from months earlier — **image `07-data-model.png`**; (7) the kill-and-resume test as the proof, linking the video timestamp. Same disclosure sentence, same tags minus Cybersecurity plus Distributed Systems.
...
>
> My hackathon project, Drawbridge, is a fleet of agents that runs enterprise vendor-security reviews end-to-end — a process that normally takes 2–4 weeks of questionnaires and PDF-reading. It blocked the injection at the gateway… and then did something I think is new: it *raised* the vendor's risk score for attempting manipulation.
>
> Built solo in 20 days on Google's Gemini Enterprise Agent Platform: Gemini 3.5 Flash, ADK, Agent Engine, Model Armor, with every decision exported as an audit binder. Total model cost per review: under $0.50.
>
...
>
> My agent fleet blocked it — then raised their risk score for trying. 🧵
...
>
> Built solo on @GoogleCloudTech's Gemini Enterprise Agent Platform: Gemini 3.5 Flash, ADK, Agent Engine, Model Armor. #AllThingsAgenticHackathon
...
| 15–25 | Score animates down, Adversarial Conduct flag, forced-escalate band | "Blocked. Logged. Risk score raised −25." |
