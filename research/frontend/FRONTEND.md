...
> **Status:** Amendments 01–03 applied to this document and to the generator scripts in `src/`. **The exported SVGs and PNGs are stale** — see §10.
...
/review/[id]/gate      S1 · gate card (G1 risk acceptance) — also renders the G2 first-contact card
...
**And one structural rule that makes S0 safe to leave open:** it is statically rendered and **no route reachable without a token may reach the model router**. The stats strip hydrates from a single cached aggregate endpoint, not from a live query and certainly not from a model call. That is what lets the hosted URL stay up through judging at min-instances 0 without the risk of a bot spending the credits (master doc risk #11).
...
**The cold start.** *(This section replaces an earlier recommendation to keep one warm dashboard instance during judging. The organisers' cost guidance is explicit that minimum instances stay at 0, and a statically rendered entry page removes the trade-off rather than making it affordable.)* **S0 is statically rendered** (Next.js SSG), with the live stats strip hydrating from one cached aggregate endpoint, so a cold start is about a second. **Keep min-instances at 0 everywhere.** Public routes serve cached Firestore reads and never call a model, so traffic cannot drain credits — a bot hitting the URL costs fractions of a cent in reads and never a Gemini token.
...
The screen the demo lives on. A single vertical event stream, because a review is a story that happened over days. Every entry expands to `goal`, `decision`, `model`, `tokens`, `cost`, `latency`, `span id` — **and `source`, the retrieval provenance: the document, page and section the passage came from.** That sixth cell is what turns a cited contradiction from a quotation into a pointer, and it is the same data the binder's section 4 prints. Right rail: Memory Bank dossier, evidence inventory with **per-document, per-filter screening verdicts** (not one CLEAN/BLOCKED pill — the filter that fired is the interesting part), findings each labelled `rule` or `model`. The stream also carries **tier-change entries**: *Re-tiered 2 → 1. Reason: the vendor's own answer to Q23 contradicts the declared data scope.* **Build note:** collapsed entries Phase 1, expansion Phase 2. The expansion is worth more than any other single component.

### S1c · The injection moment — `svg/W4-injection-moment.svg`
Same screen, mid-attack. The only full-bleed banner in the product. The blocked payload rendered in mono inside a bordered box, labelled *never re-entered into a prompt*. The **Trust Score** panel shows 56 → 31 with the −25 itemised, and an italic line naming Python rather than a model as the thing that computed it. The panel is labelled **TRUST SCORE**, never RISK SCORE, and the number *falls* — prose elsewhere may say the vendor raised your risk, but this number always drops. The blocked line **names the template and the filter that fired** — `P2 REJECTED · drawbridge-untrusted · pi_and_jailbreak MATCH_FOUND` — because `blocked` reads as a mock and the full line reads as a system. A separate panel states the consequence beyond this review: the flag persists in Memory Bank. **Build note:** Phase 2, and it is the highest-value hour of frontend work in the project.

### S1d · Gate card — `svg/W5-gate-risk-acceptance.svg`
The memo gets the largest panel because a CISO reads it and nothing else. Four fixed sections: recommendation, what drove it, mitigations, what to re-check in 90 days. A small line above it: *memo screened before display · drawbridge-output · no match* — the fleet screens what its own agents produce, and saying so on the card costs one line. Beside it, the per-domain arithmetic answering *why 71?* before it is asked, **out of 100**, with each finding carrying a `rule` or `model` provenance chip so an auditor can see which deductions were arithmetic and which were judgement. Three decisions mirroring the score bands, with conditional pre-selected because that is what the score says. Under the button: *You are signing as Elena Torres, CISO. This writes a signed, single-use approval token; no code path can set DECIDED without it.* **Build note:** Phase 1 basic, Phase 2 for the breakdown bars.

### S1e · Human gates and stalls — `svg/W6-human-gates.svg`
The **G2** first-contact card (the human act; policy **P1** at the gateway is what makes it unskippable) shows the **actual email** before it is sent, not a summary — autonomy that asks permission is a stronger claim than autonomy that doesn't. The review is parked in `GATED` with `gate_scope="contact"`, which is a different park from the `decision` scope at G1. The `NEEDS_HUMAN` card states what the fleet did *and what it refuses to do*: score on 34% coverage, or infer missing answers. Framing a stall as designed behaviour rather than an error is the difference between reading as mature and reading as broken. The DLQ panel is visible even when empty. **Build note:** Phase 1 for P1, Phase 2 for the stall card and DLQ panel.
...
| 1:05 | S1e G2 card | the actual email, the parked state, one tap |
...
- Target: S0 interactive in under 2s cold, under 500ms warm — achieved by static rendering rather than by a warm instance. **Min-instances stay at 0 on every service**, including during judging.
- **No public route calls a model.** Public read paths serve cached Firestore reads; every write — intake, approvals, portal submissions, exports — requires a signed token. Per-IP rate limits on all public routes, `max-instances 2`. This is grep-checkable and belongs in CI: no handler under `app/(public)/` may import the model router (handbook §16.5).
...
**Outstanding after Amendments 01–03: the generator sources in `src/` are current, and every `svg/W*.svg` and `png/*.png` is stale.** W2 (queue count), W3 (source cell, tier-change entry) and W5/W7 (TRUST SCORE labels and the 100-point totals) all changed. Re-run all three commands before publishing anything.
