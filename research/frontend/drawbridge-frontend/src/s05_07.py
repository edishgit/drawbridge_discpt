...
o.append(txt(1180,178,"71",40,C["warn"],700,"middle")); o.append(txt(1180,198,"TRUST SCORE",9,C["faint"],700,"middle"))
...
o.append(txt(884,252,"memo screened before display · drawbridge-output · no match",10,C["faint"],600,"end"))
...
dom=[("Data protection",20,20),("Access control",15,6),("Incident response",15,7),("Compliance posture",15,9),
     ("Subprocessors",15,9),("Business continuity",10,10),("AI-specific",10,10)]
yy=278
...
o.append(txt(960,yy+26,"Total",13,C["ink"],700)); o.append(txt(1388,yy+26,"71 / 100",13,C["ink"],700,"end"))
o.append(txt(960,yy+46,"Deterministic arithmetic over model-judged severities. No model call in scoring.",11,C["faint"],style="italic"))
o.append(txt(960,yy+64,"Findings labelled by provenance: rule (arithmetic) or model (judgement).",11,C["faint"],style="italic"))
...
# G2 first-contact card (human gate G2; policy P1 is what blocks the send)
o.append(rect(28,172,680,420,"#ffffff",C["orange"],rx=12,sw=2))
o.append(rect(28,172,680,52,C["orangebg"]))
o.append(txt(52,204,"G2 · FIRST OUTBOUND CONTACT",11,C["orange"],800))
p,w=pill(524,183,"P1 BLOCKED AT GATEWAY",C["orange"],"#ffffff",size=10); o.append(p)
...
save("W6-human-gates", svg("".join(o), "Drawbridge — the G2 contact gate and NEEDS_HUMAN"))
...
for i,(k,v) in enumerate([("Outcome","Escalated — not approved"),("Approver","Elena Torres, CISO · 16 Aug 2026 09:41 UTC"),
                          ("Trust Score","31 / 100 · band escalate"),("Adversarial conduct","Yes — attempt recorded, section 3")]):
