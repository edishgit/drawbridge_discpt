...
**Owner:** Ambrstack · 24 diagrams · v1.1, Amendments 01–03 applied

> **Status:** every `.mmd` source in `src/` is current with Amendments 01–03. **The exported PNGs and SVGs are not** — regenerate before publishing (see §26). Diagram 23 has no source file by design: it is produced by `graph_dump.py` from the workflow code itself.
...
### Failure mode
Beat 8 (`approval card — the review parks in GATED, gate_scope contact, for human gate G2`) depends on the `GATED` state carrying a scope. **This is now fixed at source:** handbook §5.2's `ALLOWED` table permits `QUESTIONNAIRE_OUT → GATED → QUESTIONNAIRE_OUT`, and `Review` carries `gate_scope`. It was a genuine runtime bug — the state validator would have raised at this exact beat, in front of a camera.
...
Each layer carries **two lines that no ordinary architecture diagram has**: *Lifetime* and *Answers*. L1 lives for a turn and answers *what am I doing right now?* L2 is forever and immutable and answers *what happened, exactly?* **L2.5 lives for the review and answers *which passage says this?*** L3 spans years and answers *what is worth knowing next time?* Those four questions are why four layers exist. Collapse any two and one question goes unanswered.

L2.5 earns its place on arithmetic rather than elegance: a Tier-1 SOC 2 runs 60–100 pages, and the hero finding depends on locating one exception note inside it. Retrieval turns that from luck into a query, makes `evidence_ref` a real pointer, and shrinks the Pro context — so the layer that improves the finding also reduces the most expensive call in the system. Only clean-stamped content is ever indexed, and if the index or the embedding call is unavailable the agent degrades to whole-document context rather than stalling.

The diagram also draws the **structured-write guard** between L2 and L3, and it is a security control rather than a schema preference: L3 is written from vendor-derived material and recalled at the *start* of every future review, before any screening runs in that review. So it takes enumerated note types, controlled vocabulary and a provenance tag, never free text — and notes supersede rather than accumulate.
...
**The domains node lists seven weights, and they now sum to exactly 100.** The original Appendix B was 20+15+15+15+10+10+10 = 95, which mattered because the binder prints the arithmetic and the bands (≥80, 60–79, <60) read as percentages of a hundred-point scale. **Resolved:** subprocessor & fourth-party chain rose from 10 to 15. The extra weight landed there deliberately — it was the domain with the least evidence behind it and it is the one the Evidence agent's subprocessor extraction is built to feed, so the points now sit where the pipeline is.

**And the number has a name.** It is the **Trust Score, 0–100, higher is safer** — not a risk score. The distinction is not cosmetic: the adversarial penalty is `raw − 25`, so an injection makes the number *fall*. Prose may say a vendor has raised your risk; the score always drops. A judge who reads a gallery card promising a raised score and then watches 56 become 31 on screen concludes the builder does not understand their own system.

**No model call appears anywhere in this pipeline except the memo.** Severity is assigned by the Evidence agent, where the model is already reading the passage; `compute_score` is arithmetic. That is what the `GAME` note says out loud — a self-improving fleet cannot game a metric it does not compute.
...
## 25 · The four corrections — resolved

These four were found by checking the diagrams against the docs. Each was a contradiction inside the source documents rather than a diagram error, and each would have become a runtime bug. **All four are now fixed at source (Amendment 01, D1–D4):**

1. **`GATED` needed a scope.** §9 parked an unapproved first contact in `GATED`; the §5.2 table only allowed `SCORED → GATED → DECIDED`. **Resolved:** `Review.gate_scope` (`contact` | `decision`), with `QUESTIONNAIRE_OUT → GATED → QUESTIONNAIRE_OUT` permitted. Without it the state validator would have raised at the 1:05 demo beat.
2. **Two topics were missing from §6.1.** `review.rescore` (published in §11.3) and `watchdog.sweep` (implied by §12's Cloud Scheduler path). **Resolved:** both are in the topic table and in `bootstrap.sh` — eleven topics. Undocumented meant untested, and an undocumented topic is one the bootstrap script does not create.
3. **`NEEDS_HUMAN` from anywhere.** §5.2's prose said any state; its table omitted `SCORED`, `GATED`, `DECIDED`, `MONITORED`. **Resolved:** the table follows the prose. Failure paths you documented but cannot execute are worse than failure paths you never wrote down.
4. **The Tier-1 rubric summed to 95.** **Resolved:** subprocessor & fourth-party chain rose from 10 to 15, so the domains total 100 and the bands are arithmetically true as percentages.

### 25.1 · Terminology, settled once

Two vocabulary collisions were doing real damage, and both are now fixed across every artefact:

**The number is the Trust Score, 0–100, higher is safer.** Never "risk score". Prose may say a vendor *raised your risk*; the score always *falls*. No diagram, wireframe, caption or line of copy now says a score was "raised" by an injection.

**Gates are not policies.** G1 risk acceptance and G2 first outbound contact are **human acts**. P1 approval token, P2 verdict-bearing stamp and P3 egress allowlist are **gateway enforcement**. G2 is a human act; P1 is the machine's inability to skip it. Nothing is called by both names — diagrams 02, 03, 08, 17, 18 and 19 were all using both for the same thing.

### 25.2 · Structural changes the amendments made to the set

| Change | Diagrams touched | Why it is not cosmetic |
|---|---|---|
| Screening now runs **before** scrubbing | 01, 03, 04 | The old order ran Model Armor's SDP filter against text whose sensitive data had already been removed — it would have returned "no match" forever, without ever erroring |
| Memory is **four** layers, with semantic retrieval inserted | 01, 06 | The hero finding stops depending on a model reading 100 pages and starts depending on a query |
| Permission matrix at **collection level**, plus `sa-armor` | 09 | A row that says "Firestore" cannot express `qa_responses` yes, `findings` no — and vague rows are how least-privilege claims quietly become false |
| **Three** gateway policies | 01, 09, 12 | Three named policies is a policy system; two is a pair of special cases |
| Approval tokens are **asymmetric** | 17 | Anything that can verify a symmetric token can also forge one, and every agent can reach the gateway |
| Re-tier transitions in the state machine | 03, 08, 19, 22 | The fleet overruling the intake form is a second decision moment, on footage already being shot |
| Late and out-of-order event behaviour | 07, 20 | Pub/Sub guarantees delivery, not sequence — and §6.5 only ever covered duplicates |
| Degrade vs fail-closed drawn as opposites | 20 | Getting this backwards is what turns a security product into a liability |
| The injection corpus and its detection rate | 04, 16, 21 | It changes the category of the claim from asserted to measured |

Two deliberate omissions across the whole set: the **Contract Clause agent** (stretch, first on the cut list) and **Veo/Lyria** (rejected in §10 as forced integrations). Add Contract Clause to 01 and 02 only if it survives Aug 24. Diagram **24** and the Target items inside 16 and 19 follow the same rule — delete them if they don't ship.

One thing the set cannot draw, so it is written here instead: **there is deliberately no edge from the public read-only surface to the model plane in diagram 01.** No route reachable without a token may reach the model router. A diagram cannot draw an absent edge; handbook §16.5 turns it into a grep you can put in CI.
...
**Outstanding right now: every export in `png/` and `svg/` predates Amendments 01–03 and is stale.** Twenty-one sources changed and one (24) is new. Regenerate the whole set rather than picking through it:
...
**Diagram 23 is not regenerated this way**, because it is not authored. It comes out of the code:
...
Re-run it after any ADK version bump. A picture generated from the repo is the strongest answer available to *"does the diagram match the code?"*, and the moment it stops matching, it says so.
